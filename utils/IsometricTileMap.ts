...
export class IsometricTileMap {
  ...
  updateZoom(zoom: number): void {
    ...
    console.log(`[IsometricTileMap] Zoom updated to: ${zoom}`);
  ...
  getContainers(): { tiles: Phaser.GameObjects.Container; sprites: Phaser.GameObjects.Container } {
    // Return empty container for sprites since they've been removed
    const emptyContainer = this.scene.add.container(0, 0);
    return {
      tiles: this.tiles,
      sprites: emptyContainer
    };
  }
} 